...
/**
 * @author user
 * @create 2020/7/13 14:19
 */
@Service
public class WoHuLTRechargeService extends AbsChannelRechargeService {
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        if (StringUtils.equals(huaFeiRechargeInfoBean.getOperator(), "联通")) {
            productRelation = queryChannelProductId("全国联通话费" + huaFeiRechargeInfoBean.getAmt() + "元", "100073");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        } else if (StringUtils.equals(huaFeiRechargeInfoBean.getOperator(), "移动")) {
            productRelation = queryChannelProductId("全国移动话费" + huaFeiRechargeInfoBean.getAmt() + "元", "100073");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        } else {
            productRelation = queryChannelProductId("全国电信话费" + huaFeiRechargeInfoBean.getAmt() + "元", "100073");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        }
