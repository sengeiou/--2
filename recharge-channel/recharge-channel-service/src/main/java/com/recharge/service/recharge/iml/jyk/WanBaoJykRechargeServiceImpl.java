...
@Service
public class WanBaoJykRechargeServiceImpl extends AbsChannelRechargeService {
...
//      请求地址：http://129.204.60.17:8107/MainServiceBusiness/SendProductChargeInfoByNo
        ...
        productName.append("全国油卡直冲").append(jykRechargeInfoBean.getAmt()).append("元");
        ProductRelation productRelation = queryChannelProductId(productName.toString(), "100052");
        String productid = productRelation.getChannelProductId();

        String orderId = channelOrder.getChannelOrderId();
        String chargeNumber = jykRechargeInfoBean.getAccount();
        String retailMoney = jykRechargeInfoBean.getAmt();
        String number = String.valueOf(jykRechargeInfoBean.getNumber());
//        代理商请填写2，此处务必填写2，否则可能造成提交订单号重复。
        ...
        try {
            ...
            logger.info("send jyk recharge request params:{}", requestUrl);
            String responseBody = HttpClientUtils.invokeGetHttp(requestUrl, "utf-8", 5000);
            logger.info("send jyk recharge response :{}", responseBody);
...
//      代理商信息查询地址：http://129.204.60.17:8107/MainServiceBusiness/GetAgentInfo
        ...
        try {
            ...
            logger.error("send queryBalance error", e);
            return BigDecimal.ZERO;
        }
    }
