 ...
 * @create 2021/3/1 13:31
 */
@Service
public class WoHuHaveVoucherJykRechargeServiceImpl extends AbsChannelRechargeService {
    private Logger logger = LoggerFactory.getLogger(getClass());

    private String channelId = "100146";
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        try {
            ...
            logger.error("{}沃虎加油卡(有透传)查询订单接口请求报错的参数{}", channelOrder.getChannelOrderId(), e);
    ...
    public ProcessResult query(Channel channel, ChannelOrder channelOrder) {
        ...
        map.put("account", account);
        try {
            logger.info("沃虎加油卡(有透传)订单查询接口请求的参数:{}", JSONObject.toJSONString(map));
            String responseBody = HttpClientUtils.invokeJsonString(url, new StringEntity(JSON.toJSONString(map), "UTF-8"), "", "utf-8", 5000);
            logger.info("沃虎加油卡(有透传)订单查询接口响应的参数:{}", responseBody);
            String result_code = JSONObject.parseObject(responseBody).getString("result_code");
            if (StringUtils.equals("00000", result_code)) {
            ...
            logger.info("沃虎加油卡(有透传)订单查询接口报错的参数:{}", e.getMessage());
    ...
    void test() {
        ...
        channel.setConfigInfo("{rechargeUrl:\"http://frp.fjwolf.com:9090/frp/api/order/recharge\",app_id:\"C29E3F6EBF3DB6A782D71E4316C24325\",querybalanceUrl:\"http://frp.fjwolf.com:9090/frp/api/merchant/querybalance\",queryUrl:\"http://frp.fjwolf.com:9090/frp/api/order/querysts\",password:\"32ED158A500CB8A1C2255438CE8C31D6\"}");
        ChannelOrder channelOrder = new ChannelOrder();
        channelOrder.setChannelOrderId("cs202103081630");
