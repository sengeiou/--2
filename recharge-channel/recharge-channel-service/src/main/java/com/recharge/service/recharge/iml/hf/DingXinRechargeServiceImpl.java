...
@Service
public class DingXinRechargeServiceImpl extends AbsChannelRechargeService {

    private Logger logger = LoggerFactory.getLogger(getClass());

    private Map<String, String> errorMap = new HashMap<>();

    public DingXinRechargeServiceImpl() {
        errorMap.put("0", "无错误");
        ...
        try {
            ...
            if (StringUtils.equals(error.getStringValue(), "0")) {
                if (StringUtils.equals(state.getStringValue(), "1")) {
                    Element flowNo = root.element("flowNo");
                    if (flowNo != null) {
                        channelOrder.setOutChannelOrderId(flowNo.getStringValue());
                ...
                } else if (StringUtils.equals(state.getStringValue(), "9999")) {
                    return new ProcessResult(ProcessResult.UNKOWN, "未知错误，跟供货商核实");
                } else {
