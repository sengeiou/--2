...
/**
 * @author user
 * @create 2020/11/3 17:48
 */
@Service
public class YuanFeiCGRechargeService extends AbsChannelRechargeService {
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        String gascardtel = "";
        String gascardname = "";
        System.out.println("userid=" + userid + "&productid=" + productid + "&price=" + price + "&num=" + num + "&mobile=" + mobile + "&spordertime=" + "&sporderid=" + sporderid + "&key=" + key);
        ...
        try {
            logger.info("yuanFeiCG send recharge request params:{}", JSONObject.toJSONString(map));
            String responseBody = HttpClientUtils.invokePostHttp(url, map, "utf-8", 5000);
            logger.info("yuanFeiCG send recharge response :{}", responseBody);
            ...
            if (StringUtils.equals("0", resultno)) {
                return new ProcessResult(ProcessResult.SUCCESS, "等待充值");
    ...
    public ProcessResult query(Channel channel, ChannelOrder channelOrder) {
        ...
        map.put("sporderid", sporderid);
        try {
            logger.info("yuanFeiCG send query request params:{}", JSONObject.toJSONString(map));
            String responseBody = HttpClientUtils.invokePostHttp(url, map, "utf-8", 5000);
            logger.info("yuanFeiCG query response :{}", responseBody);
