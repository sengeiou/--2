 ...
 * @create 2021/1/4 16:55
 */
@Service
public class DingChangRechargerServiceImpl extends AbsChannelRechargeService {
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        map.put("sign", sign);
        try {
            logger.info("{}鼎昌,发送充值的参数:{}", rechargeOrderBean.getOrderId(), JSON.toJSONString(map));
            String responseBody = HttpClientUtils.invokePostHttp(url, map, "utf-8", 5000);
            logger.info("{}鼎昌,接收充值的参数:{}", rechargeOrderBean.getOrderId(), JSON.toJSONString(responseBody));
    ...
    public ProcessResult query(Channel channel, ChannelOrder channelOrder) {
        ...
        map.put("sign", sign);
        try {
            logger.info("{}鼎昌,发送查询的参数:{}", channelOrder.getChannelOrderId(), JSON.toJSONString(map));
            String responseBody = HttpClientUtils.invokePostHttp(url, map, "utf-8", 5000);
            logger.info("{}鼎昌,接收查询的参数:{}", channelOrder.getChannelOrderId(), JSON.toJSONString(responseBody));
            String code = JSONObject.parseObject(responseBody).getString("code");
            if (StringUtils.equals(code, "1")) {
                String status = JSONObject.parseObject(responseBody).getString("status");
            ...
            logger.error("{}鼎昌,发送查询出错的参数:{}", channelOrder.getChannelOrderId(), e.getMessage());
