 ...
 */
@Service
public class ManFanFHRechargeSercice  extends AbsChannelRechargeService {
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        try {
            logger.info("{},ManFanFH发送充值的参数:{}",rechargeOrderBean.getOrderId(), JSON.toJSONString(requestMap));
            String responseBody = HttpClientUtils.invokePostHttp(requestUrl, requestMap, configJSONObject.getString("encoding"));
            logger.info("{},ManFanFH收到充值的响应:{}",rechargeOrderBean.getOrderId(),responseBody);
            ...
            logger.error("{}ManFanFH send error",rechargeOrderBean.getOrderId(),connectException);
            return new ProcessResult(ProcessResult.FAIL,"提交失败");
        }catch (Exception e) {
            logger.error("{}ManFanFH send error",rechargeOrderBean.getOrderId(),e);
    ...
    public ProcessResult query(Channel channel, ChannelOrder channelOrder) {
        ...
        try {
            logger.info("ManFanFH发送查询的参数:{}",JSON.toJSONString(requestMap));
            String responseBody = HttpClientUtils.invokePostHttp(requestUrl, requestMap, configJSONObject.getString("encoding"));
            logger.info("ManFanFH收到查询的响应:{}",responseBody);
