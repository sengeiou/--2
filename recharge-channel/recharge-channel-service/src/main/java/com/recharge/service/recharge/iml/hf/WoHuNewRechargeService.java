...
/**
 * @author user
 * @create 2020/11/6 10:30
 */
@Service
public class WoHuNewRechargeService extends AbsChannelRechargeService {
    ...
    public ProcessResult recharge(Channel channel, ChannelOrder channelOrder, RechargeOrderBean rechargeOrderBean) {
        ...
        if (StringUtils.equals(huaFeiRechargeInfoBean.getOperator(), "联通")) {
            productRelation = queryChannelProductId("全国联通话费"+huaFeiRechargeInfoBean.getAmt() + "元" , "100103");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        } else if (StringUtils.equals(huaFeiRechargeInfoBean.getOperator(), "移动")) {
            productRelation = queryChannelProductId("全国移动话费"+huaFeiRechargeInfoBean.getAmt()+"元", "100103");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        } else {
            productRelation = queryChannelProductId("全国电信话费"+huaFeiRechargeInfoBean.getAmt()+"元", "100103");
            if (productRelation == null) {
                return new ProcessResult(ProcessResult.FAIL, "提交失败");
            }
        }
        ...
        try {
            logger.info("woHuNew send recharge request params:{}", JSONObject.toJSONString(requestMap));
            String responseBody = HttpClientUtils.invokePostString(url, new StringEntity(JSON.toJSONString(requestMap)), "UTF-8", 5000);
            logger.info("woHuNew send recharge response :{}", responseBody);
            ...
            logger.error("{}woHuNew send error{}", rechargeOrderBean.getOrderId(), e);
    ...
    public ProcessResult query(Channel channel, ChannelOrder channelOrder) {
        ...
        requestMap.put("account", account);
        try {
            logger.info("woHuNew send query request params:{}", JSONObject.toJSONString(requestMap));
            String responseBody = HttpClientUtils.invokePostString(queryUrl, new StringEntity(JSON.toJSONString(requestMap)), "UTF-8", 5000);
            logger.info("woHuNew send query response :{}", responseBody);
            JSONObject jsonObject = JSONObject.parseObject(responseBody);
            String code = jsonObject.getString("result_code");
